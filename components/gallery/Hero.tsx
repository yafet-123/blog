...
const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <div className={`text-white flex flex-col lg:flex-row items-center justify-between px-5 lg:px-20 w-full h-full lg:h-[35rem] pt-32 bg-[#050d1a]`}>
       <div className="w-full lg:w-1/2 lg:px-20">
        <Slider {...settings}>
          {HeroImages.map((data, index) => (
            <div key={index} className="relative w-full h-96 lg:h-[500px] clip-curve lg:mt-32">
              <Image
                layout="fill"
                src={data.image}
